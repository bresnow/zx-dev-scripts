...
const InitializeSwarm = async () => {
  ...
  try {
    ...
    let lists = advAdd.map(({ ip, label }, index) => {
      console.log(chalk.blueBright([`Type "${chalk.yellowBright.bold(label)}" OR "${chalk.yellowBright.bold(index)}" to use `] + chalk.green.italic.bold(ip)) + '\n')
      return `${index},${label},${ip}`
    })

    if (lists.length === 1) {
      let [_index, _label, ip] = lists[0].split(/,/g)
      console.log(chalk.blueBright(`Only one ip -- Using default advertising address ` + chalk.green.italic.bold(ip)))
      await $`docker swarm init --advertise-addr ${ip}`
      process.exit(0)
    }
    let answer = await question(chalk.underline.italic.bold(`Choose advertising address\n\n`));
    for (let k = 0; k < lists.length; k++) {
      ...
      if (answer.trim() === (label || index)) {
        ...
        ip = lists[0][2]
        console.log(chalk.yellow(`Invalid option ... Using default address ` + chalk.green.italic.bold(ip)))
        await $`docker swarm init --advertise-addr ${ip}`
        process.exit(0)
      }
    }
