...
//net-tools
await sleep(1000);
let shouldInstallNetTools = await question(
  "Do you want to install net-tools? [y/n] "
);
...
//openssh-server
await sleep(1000);
let shouldInstallOpenSSH = await question(
  "Do you want to install openssh-server? [y/n] "
);
...
if (shouldInstallUFW.toLowerCase() === "y") {
  ...
  console.log(
    "Skipping ufw installation. Remote ssh to this computer may not work if you did not install openssh server"
  );
}
...
// docker
let shouldInstallDocker = await question(
...
// docker-compose
let shouldInstallDockerCompose = await question(
